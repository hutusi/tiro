 ...
 * `popup.html?state=<name>` in a development build (`bun run build:dev`).
...
export const fixtures: Record<string, PopupState> = {
  ...
  failed: {
    ...
    phase: "failed",
    problem: {
      text: "GitHub token is invalid or expired — update it in Settings.",
      error: true,
    },
  },
  unconfigured: {
    ...
    configured: false,
    problem: {
      text: "Set your GitHub repository and token in Settings first.",
      error: true,
    },
  },
  pdf: {
    ...
    preview: null,
    problem: {
      text: "This is a PDF, not a web page — its text is rendered by a plugin the extension cannot read.",
      error: true,
    },
  },
  "arxiv-offer": {
  ...
  "arxiv-abstract": {
    ...
    note: "arXiv has no HTML full text for this paper; clipping its abstract page.",
  },
};
