...
- **Collections are the only vault content a person writes** (`collections/<id>.md`, favorites being `favorites`; ADR 0029) — kept out of article frontmatter because the processor rewrites `index.md` on every run. A collections push must never start processing: the vault's `publish.yml` only dispatches a deploy, and no processor glob reaches `collections/` — the processor reads `articles/`, and `inbox/` for saved links (ADR 0034).
...
| Path | What |
| --- | --- |
| `packages/shared` | **The content contract — keystone.** Zod frontmatter schemas, slug rules, block alignment, collection documents, `tiro.yml` config schema |
...
5. **All clipped HTML passes rehype-sanitize** in `apps/site/src/lib/render.ts` before reaching the public site. Never reintroduce `allowDangerousHtml` at the stringify stage; extend the sanitize schema instead if legitimate tags get stripped. **Anything that generates its own markup runs *after* the sanitize step** — Shiki, KaTeX, and the anchor scoping that reconciles a clobbered `id` with the link pointing at it (ADR 0024) — so the allowlist never has to admit the `class`/`style` those emit, which would admit them from clipped markup too (ADR 0009). The anchor pass sits *before* the generators for the mirror reason: it may only ever touch clipped markup.
6. **`@tiro/shared`'s root export stays browser-safe** (it is bundled into the extension). Anything touching `node:` APIs goes in a subpath export (`@tiro/shared/config`). Browser-safe is not *worker*-safe: the root pulls in remark, whose browser build touches `document` at load, and the extension's service worker has no DOM — importing the root there kills the worker on registration while every unit test passes (Bun resolves the DOM-free build). **Code the worker reaches imports `@tiro/shared/documents`**, never the root; the extension `build` loads `dist/background.js` without a DOM and fails if it throws. `@tiro/clip` is DOM code by design, so the same rule covers it: the worker may import its types (erased from the bundle), never its values.
...
- The site reads the vault from the filesystem in `apps/site/src/lib/vault-read.ts` — its one reader — not through Astro's content layer (ADR 0020: that layer measured images the site never uses, and one unmeasurable asset failed every deploy). `TIRO_VAULT_DIR` still selects the vault (default `fixtures/vault`, found by searching upward rather than by counting `..`, since this code is bundled for the build). **Two different guards, for two different failures, and both must stay** (ADR 0006): `vault.ts` throws when the configured base has no `articles/` at all, and `articles.ts` throws on a build that found zero articles under one that does. Neither is redundant — the first cannot see an `articles/` holding nothing, and the second cannot tell a wrong path from an empty vault.
...
- [docs/adr/](docs/adr/) — numbered decision records. A reversed decision gets a superseding ADR, not a silent edit.
