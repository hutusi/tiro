...
- **Collections are the only vault content a person writes** (`collections/<id>.md`, favorites being `favorites`; ADR 0029) — kept out of article frontmatter because the processor rewrites `index.md` on every run. A collections push must never start processing: the vault's `publish.yml` only dispatches a deploy, and every processor glob stays rooted at `articles/`.
