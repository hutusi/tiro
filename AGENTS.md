...
6. **`@tiro/shared`'s root export stays browser-safe** (it is bundled into the extension). Anything touching `node:` APIs goes in a subpath export (`@tiro/shared/config`). Browser-safe is not *worker*-safe: the root pulls in remark, whose browser build touches `document` at load, and the extension's service worker has no DOM — importing the root there kills the worker on registration while every unit test passes (Bun resolves the DOM-free build). **Code the worker reaches imports `@tiro/shared/documents`**, never the root; the extension `build` loads `dist/background.js` without a DOM and fails if it throws.
