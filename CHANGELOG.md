...
- **Unlisted articles.** `unlisted: true` in an article's frontmatter keeps it
  ...
  URL with a `未公开` label and a `noindex, nofollow` robots tag. The vault's
  only levers until now were deleting the article or moving it out of
  `articles/`, both of which take the URL with it. Set by hand, and carried
  forward by both the processor and a re-clip, so hiding an article stays
  hidden. It hides an article from anyone
  browsing the site, not from anyone who knows the source URL — slugs are
  computable from it — so it is not access control (ADR 0017).
...
- **The site follows the "Later Reader" design** (ADR 0014). Cream paper,
  ...
  toolbar — layout, text size, paper — and a title block with domain, author
  and a build-time reading time; a body that opens with its own H1 no longer
  shows the title twice. Reading preferences (paper cream/white/dark, text
  size, default layout, list view) live in `localStorage` on a new
  `/settings/` page; the header's light/dark toggle is gone and its stored
  choice migrates. Favicon, touch icon and social card are the oxblood "T"
  monogram, regenerated by `apps/site/scripts/brand.ts`. The extension's
  popup and icon are unchanged until their own redesign.
...
[Unreleased]: https://github.com/hutusi/tiro/compare/v0.9.0...HEAD
[0.9.0]: https://github.com/hutusi/tiro/compare/v0.8.0...v0.9.0
